...
//    so frame t is centered at sample t*hop_length, which is why the
//    frame count is 1 + floor(N/hop) (313 frames for this firmware's
//    160000-sample/16kHz clip). The pad
...
//    uses a SINGLE global max over the entire (128, TARGET_T) mel power
//    array as the reference — not a per-frame or per-bin max. That means the
...
// mel_power holds the full (128, TARGET_T) linear-power spectrogram — kept
...
// 128*313*4 bytes = ~155 KB — allocated from PSRAM (accessed sequentially
// only, unlike fft_buf below, so PSRAM's latency penalty doesn't bite here
// the way it did for the FFT working buffer).
...
int mel_spectrogram_compute(const float *audio, mel_features_t *out) {
    ...
    // complex points (16KB for N_FFT=2048) -- MALLOC_CAP_INTERNAL (fast
    // SRAM), not PSRAM. Real-hardware measurement (2026-08-10) found
    // mel_spectrogram_compute() taking ~11.5s per call, ~85% of total
    // edge-model latency, with this buffer in PSRAM. FFT butterflies are a
    // scattered/strided access pattern run against this buffer twice per
    // frame (dsps_fft2r_fc32 + dsps_bit_rev_fc32) x TARGET_T (313) frames
    // -- close to the worst case for PSRAM's much higher random-access
    // latency vs. internal SRAM. The buffer is small enough (16KB) to
    // comfortably fit in internal SRAM alongside everything else; only
    // s_mel_power (the ~155KB (128,313) result buffer, sequential access
    // only) still needs PSRAM.
    float *fft_buf = (float *)heap_caps_malloc(2 * N_FFT * sizeof(float), MALLOC_CAP_INTERNAL);
    if (fft_buf == NULL) {
        fft_buf = (float *)heap_caps_malloc(2 * N_FFT * sizeof(float), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    }
    if (fft_buf == NULL) {
    ...
    // Temporary diagnostic instrumentation (2026-08-10): moving fft_buf to
    // internal SRAM didn't change total latency at all (11.45s vs 11.53s),
    // ruling out PSRAM bandwidth as the cause -- splitting the per-frame
    // loop into its three phases to find which one actually dominates
    // before changing anything else.
    int64_t t_window_us = 0, t_fft_us = 0, t_filterbank_us = 0;
    ...
    for (int t = 0; t < TARGET_T; t++) {
        ...
        // Apply the precomputed librosa mel filterbank -- SPARSE (real
        // hardware measurement, 2026-08-10: the dense (128,1025) matmul
        // took ~10.9s/clip, ~95% of total edge-model latency; each mel
        // filter's nonzero support is a single contiguous triangular band
        // averaging ~14.7 of 1025 bins, verified computationally in
        // generate_firmware_data.py, not assumed -- see mel_filterbank_data.h).
        for (int m = 0; m < N_MEL; m++) {
            int start = g_mel_fb_start[m];
            int len = g_mel_fb_len[m];
            const float *weights = &g_mel_fb_weights[g_mel_fb_offset[m]];
            float acc = 0.0f;
            for (int i = 0; i < len; i++) {
                acc += weights[i] * power[start + i];
            }
        ...
        t_window_us += (p1 - p0);
        t_fft_us += (p2 - p1);
        t_filterbank_us += (p3 - p2);
    }

    ESP_LOGI(TAG, "mel phase breakdown: window=%.1fms fft+bitrev=%.1fms filterbank_matmul=%.1fms "
                  "(%d frames)",
             (double)t_window_us / 1000.0, (double)t_fft_us / 1000.0,
             (double)t_filterbank_us / 1000.0, TARGET_T);
