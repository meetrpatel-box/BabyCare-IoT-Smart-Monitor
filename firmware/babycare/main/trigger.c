...
#define STAGE_A_TRIGGER_MULT      (300 / 100.0f)       // 3.0x noise floor
#define STAGE_A_ABS_MIN_RMS       (250 / 100000.0f)    // 0.0025 absolute min
...
void trigger_init(int sample_rate_hz) {
    ...
    ESP_LOGI(TAG, "trigger_init: Stage A energy gate only (mult=%.2fx floor, abs_min=%.5f) -- "
                  "actual cry/not-cry discrimination is now done by the ML edge model, "
                  "not this DSP stage. PROVISIONAL, needs broader calibration.",
